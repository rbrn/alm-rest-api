...
public class Client
{
    ...
    public TestInstance updateTestInstance(TestInstance testInstance) throws Exception {
        Log.info(String.format("Updating TestInstance ... (test-instance-id = %s)", testInstance.id()));
        TestInstance updateTestInstance = Dao.updateTestInstance(testInstance);
        Log.info(String.format("Updated TestSet (test-instance-id = %s, '%s')", testInstance.id(), testInstance.name()));
        return updateTestInstance;
    }
    ...
    public Run createRun(TestInstance testInstance, Test test) throws Exception
    {
        ...
        prepRun.status(Run.STATUS_PASSED);
        prepRun.host(hostName().toUpperCase());
        prepRun.comments(test.description());
        prepRun.duration("19");
        prepRun.classification("Internal");
        prepRun.name("Script run" +dateFormat.format(new Date()));
    ...
    public void createRunSteps(Run run, RunSteps runSteps) throws Exception
    {
        ...
        for (RunStep runStep : runSteps.entities())
        {
            ...
            prepRunStep.expected(runStep.expected());
            prepRunStep.description(runStep.description());
            prepRunStep.testId(runStep.testId());
            prepRunStep.actual(runStep.actual());
            prepRunStep.executionTime(runStep.executionTime());
    ...
    public RunSteps loadRunSteps(String id) throws Exception {
       return Dao.readRunSteps(id);
    }

    public static RunStep updateRunStep(RunStep runStep) throws Exception {

        RunStep updatedRunStep =   Dao.updateRunStep(runStep);

        return updatedRunStep;
    }

    public TestInstance loadTestInstanceByName(String testInstanceName) throws Exception {

        Log.info(String.format("Loading TestInstances ... (name = %s)", testInstanceName));

        TestInstance testInstance = Dao.readTestInstanceByName(testInstanceName);

        Log.info(String.format("Loaded %d TestInstance", testInstance.toString()));
